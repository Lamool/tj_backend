...
class 상위클래스A {
    // 생성자 : 코드가 없으면 컴파일시 자동으로 디폴트생성자 생성됨.
}
...
class 상위클래스B {
    // 생성자 : 생성자 코드를 구현하면 컴파일시 자동으로 디폴트 생성자 생성 안 됨.
...
class 상위클래스C {
    // 생성자 : 2개 (디폴트, 매개변수가 있는), 오버로딩 : 매개변수의 순서, 개수, 타입 따라 여러 개 생성자를 구현 (매개변수 이름 다르다고 되는 거 아님)
    상위클래스C() {}
    상위클래스C(boolean a) {}
}

class 하위클래스C extends 상위클래스C {
    하위클래스C() { }    // 상위클래스C에서 디폴트 생성자 존재하므로 super() 자동으로 호출
}
// ============================================================== //
