/*
	제약조건
        ...
        4. auto_increment : 레코드 삽입시 해당 필드내 자동번호 부여 (oracle은 지원하지 않음)
*/
...
# 제약조건
# 1. not null : 해당 필드에 null 값을 포함하지 않는다.
		-- (레코드[insert] 등록시 해당 필드에 대입 값이 없을 때 기본적으로 null이 대입
create table t1( tno int not null, tname varchar(10) );
	# 레코드/행/튜플 추가
		# 만약에 values(값1, 값2) 만약에 앞에 필드명을 생략시 필드명 순서대로 값 대입
insert into t1 values(1, '유재석');
		# 만약에 values(값1, 값2) 만약에 앞에 필드명을 작성시 명시된 순서대로 값 대입
insert into t1(tno, tname) values(2, '강호동');
insert into t1(tname, tno) values('하하', 3);

insert into t1(tname) values ('서장훈');  # 오류있네? 값을 대입하지 않은 필드가 not null이기 때문에
insert into t1(tno) values (4);			 # 오류없네? 값을 대입하지 않은 필드가 not null 하지 않았기 때문에
...
delete from t4 where tno = 3;	# tno가 '3'인 레코드 삭제
	# 레코드 삭제후 레코드 생성 : 삭제된 자동번호는 더이상 생성되지 앟고 마지막 번호에 이어서 1씩 증가
...
# 6. foreign key : 외래(foreign)키, 다른 테이블내 기본키를 참조하는 키, 참조 하는 키
create table t6(
	title text,
	tno_fk int,
...
# 워크벤치 상단 메뉴 -> database -> Reverse Engineer (ctrl+R) -> 다음 -> DB 선택 -> 다음
