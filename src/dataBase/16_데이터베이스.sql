...
select mid a1 from member m;	# as 생략 가능

# 4. 조건절 [ where 조건식 ]
select * from member where mname = '블랙핑크';	# mname의 필드값이 '블랙핑크'인 레코드 검색
select * from member where mnumber = 4;			# mnumber의 필드값이 '4'인 레코드 검색alter
...
select * from member where mheight >= 165 or mnumber > 6;		# mheight의 필드값이 165 이상 이거나 mnumber의 필드값이 6 초과인 레코드 검색
select * from member where maddr = '경기' or maddr = '전남' or maddr = '경남';
select * from member where maddr in('경기', '전남', '경남');
select * from member where maddr not in('경기', '전남', '경남');
...
select * from member where mphone1 = null;		# null 검색이 안 된다.***주의.
...
# 5. 조회 결과의 레코드를 필드 기준으로 정렬
	# order by 필드명[생략] asc : 필드 기준으로 레코드를 오름차순 정렬. 작은수 -> 큰수, 과거날짜 -> 최근날짜
    # order by 필드명 desc     : 필드 기준으로 레코드를 내림차순 정렬, 큰수 -> 작은수, 최근날짜 -> 과거날짜 
select * from member order by mdebut;
select * from member order by mdebut asc;
select * from member order by mdebut desc;
	# 정렬필드 기준이 2개 이상일 때 주의할 점
		# 첫 번재 필드 정렬 후 첫 번째 필드내 동일한 값이 있을 경우 동일한 값 중에서 두번째 정렬 실행
...
select * from member limit 0, 3;	# 0(첫번째) 레코드부터 3개 출력 제한
select * from member limit 1, 2;
select * from member order by mheight desc limit 3;		# meheight이 필드값이 상위 3개 레코드 검색

# 7. 검색된 필드의 중복 제거 : distinct
select maddr from member;			# 모든 주소 검색
select distinct addr from member 	# 모든 주소에서 중복 제거 검색

# select에서 where절, order절, limit절 동시에 작성시에는 순서 존재
