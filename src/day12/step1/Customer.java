...
public class Customer {
    // 1. 멤버변수 : 객체내 속성 값을 저장할 변수, 접근제한자 static 타입 변수명
        // 접근제한자
        // private : 다른 클래스로부터 접근제한, 현재 클래스에서만 호출이 가능하다.
    ...
    // 2. 생성자 : 객체 생성(new) 시 사용되는 메소드, 반환타입이 없다.
        // 오버로딩 : 생성자명은 동일하지만 매개변수 개수, 타입, 선언순서가 다르면 여러 개 선언가능
        // 클래스내 생성자가 1개도 없으면 기본(default) 생성자 자동으로 생성되지만
            // 생성자가 1개 이상 존재하면 기본생성자는 자동으로 생성되지 않는다.
        // 주로 객체 생성시 멤버변수의 초기화 또는 유효성검사를 진행할 때.
    public Customer() {
    ...
    public int calcPrice(int price) {
        ...
        // this란? 매개변수와 멤버변수 구분하기 위해, 해당 함수를 실행한 객체를 식별용   (함수는 공유해서 사용하기 때문에, 누가 실행했느냐를 this로 구분)
